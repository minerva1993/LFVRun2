...
void makePlot_inv(TFile * f, TString name, TString n, TString i, float x1, float y1, float x2, float y2){
  ...
  h_n->SetAxisRange(0,0.15,"Y");
  h_i->SetAxisRange(0,0.15,"Y");
  h_n->GetXaxis()->SetTitle("inv mass of top(toLQ)");
...
void plots_inv(){
  ...
  setTDRStyle();
    ...
    makePlot_inv(S,"h_inv_mass_top_toLQ_S","h_inv_mass_top_toLQ_nfW","h_inv_mass_top_toLQ_ifW",0.2,0.7,0.5,0.85 );	
//		makePlot_cpt(BG,"h_leading_good_cjet_W_BG","h_leading_good_c_hadron_nfW_pt","h_leading_good_c_hadron_ifW_pt",0.5,0.7,0.8,0.85 );
}
