...
void makePlot_good_hadron(TFile * f, TString b, TString c, TString lf, float x1=0, float x2=10, float y1=0, float y2=300){
  ...
  h_c->SetLineColor(4);
  h_lf->SetLineColor(8);
  h_b->SetAxisRange(0,10,"X");
  h_c->SetAxisRange(0,10,"X");
  h_lf->SetAxisRange(0,10,"X");
  h_b->SetAxisRange(0,50000,"Y");
  h_c->SetAxisRange(0,50000,"Y");
  h_lf->SetAxisRange(0,50000,"Y");
  h_b->GetXaxis()->SetTitle("Number of Jets");
  h_b->GetYaxis()->SetTitle("Number of Events");
  h_b->Draw("HIST");
  h_c->Draw("SameHIST");
  h_lf->Draw("SameHIST");
...
void plots_good_hadron(){

  setTDRStyle();

  TFile * file = new TFile("tt_f_DeepFlav_2018.root");

  makePlot_good_hadron(file,"h_n_good_b_hadron","h_n_good_c_hadron","h_n_good_lf_hadron");
}
